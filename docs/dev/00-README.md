...
이 디렉토리는 **이 코드베이스를 인수받은 개발자**가 *왜 이렇게 설계되어 있는지*를 이해할 수 있게 정리한 문서입니다. 단순히 *어디를 만지는지*([docs/contribution/](../contribution/)) 또는 *오늘 무엇을 할지*([OPERATIONS.md](../../OPERATIONS.md))가 아니라, **시스템의 결정들이 어떤 제약과 트레이드오프에서 나왔는지** 설명합니다.
